...
import { tokenForExpression } from './trans.ts'
import type {
  FrameworkAdapter,
  MacroNames,
  ParsedFile,
 ...
 * turns it into a `Message`, rejects every use that cannot compile, and
 * notes what `transform()` will have to rewrite besides the messages.
 *
 * Shared with `extract()`, which stops here.
 */
...
/**
 * Everything the collectors below share about one script: the macro
 * bindings, the messages found so far, and the identifier nodes that are
 * legitimate macro uses - so the misuse guard can tell them from misuse.
 */
interface Context {
  parsed: ParsedSource
  code: string
  ...
  names: MacroNames
  adapter: FrameworkAdapter
  /** Local names bound to each macro export. */
  locals: Set<string>
  pluralLocals: Set<string>
  hookLocals: Set<string>
  componentLocals: Set<string>
  /** Local names bound to the runtime `useLocale`. */
  localeReadLocals: Set<string>
  messages: Message[]
  /** The `// i18n:` note attached to a line, if any. */
  descriptionFor: (line: number) => string | undefined
  tagNodes: Set<unknown>
  allowedPluralNodes: Set<unknown>
  allowedHookNodes: Set<unknown>
  allowedComponentStarts: Set<number>
}

/** What `collectHooks` learned, which the later collectors dispatch on. */
interface Hooks {
  hookCalls: HookCall[]
  localeAliases: Array<{ start: number; end: number; text: string }>
  reusableLocaleRead: string | undefined
  /** Names declared by `const x = useI18n()`, anywhere in the file. */
  localeVars: Set<string>
  /** The hook variable in scope at `offset`, if any. */
  localeVarAt: (offset: number) => string | undefined
  /** The scope declaring hook variable `name` that contains `offset`. */
  hookScopeAt: (offset: number, name: string) => Scope | undefined
}

/** The innermost function containing `offset`, of those that qualify. */
function innermost(offset: number, of: Scope[]): Scope | undefined {
...
function analyzeContext(
  ...
  const { macroImports } = bindings

  if (
    bindings.locals.size === 0 &&
    bindings.pluralLocals.size === 0 &&
    bindings.hookLocals.size === 0 &&
    bindings.componentLocals.size === 0
  ) {
  ...
  const context: Context = {
    ...
    pluralLocals: bindings.pluralLocals,
    hookLocals: bindings.hookLocals,
    componentLocals: bindings.componentLocals,
    localeReadLocals: bindings.localeReadLocals,
    messages: [],
    descriptionFor: noteReader(parsed, code),
    tagNodes: new Set(),
    allowedPluralNodes: new Set(),
    allowedHookNodes: new Set(),
    allowedComponentStarts: new Set(),
  }

  const hooks = collectHooks(context)
  collectTags(context, hooks)
  collectPlurals(context, hooks)
  collectTrans(context, hooks)
  guardMisuse(context, hooks)
    ...
    if (current.start < previous.end) {
      throw new Error(
        `best-i18n: nested messages in ${filename} are not supported (at ` +
          `offset ${current.start}). A ${names.tag}\`\`, ${names.plural}() or ` +
          `<${names.component}> cannot contain another one.`,
      )
    }
  }
  ...
  return {
    messages,
    hookCalls: hooks.hookCalls,
    localeAliases: hooks.localeAliases,
    reusableLocaleRead: hooks.reusableLocaleRead,
    macroImports,
  }
}

/**
 * Which local names are the macros, and which imports brought them in. Also
 * where a file is rejected outright: a namespace import, or another
 * framework's macro.
 */
 ...
 */
function collectHooks(context: Context): Hooks {
  const {
    parsed,
    code,
    filename,
  ...
  return {
    ...
    reusableLocaleRead,
    localeVars,
    localeVarAt: (offset) => innermost(offset, withLocaleVar)?.localeVar,
    hookScopeAt,
  }
}
...
function collectPlurals(context: Context, hooks: Hooks): void {
  const {
    parsed,
    code,
    filename,
  ...
  const { plural: pluralName } = context.names
  const { localeVarAt } = hooks
  ...
  walk(parsed.program, (node) => {
    ...
    const localeVar = localeVarAt(start)
...
/** `<Trans>...</Trans>`, whatever the framework spells it as. */
function collectTrans(context: Context, hooks: Hooks): void {
  const {
    parsed,
    code,
    filename,
    ...
    componentLocals,
    messages,
    allowedComponentStarts,
    descriptionFor,
  } = context
  const { component } = context.names
  const { localeVarAt } = hooks
  ...
  walk(parsed.program, (node, parent) => {
    ...
    const localeVar = localeVarAt(match.start)
...
/**
 * Any other reference to a macro binding cannot be compiled: `const f = t`,
 * `foo(t)`, or a local that shadows it. Fail here instead of leaving a
 * runtime throw for someone to find in production.
 */
function guardMisuse(context: Context, hooks: Hooks): void {
  const {
    parsed,
    filename,
    locals,
  ...
  const { localeVars, hookScopeAt } = hooks
