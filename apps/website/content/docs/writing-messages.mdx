...
Svelte uses the same component and the same catalog placeholders, from
`best-i18n/svelte/macro`. The compiler emits `{#if}` around the locale
branches instead of a JSX fragment — Svelte cannot put elements inside
`{...}`. See [Svelte](/docs/integrations/svelte).
