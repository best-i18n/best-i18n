...
The Svelte and Solid families add two runtime libraries that have no
compile step at all:

|                            | svelte-i18n                                   | @solid-primitives/i18n                    |
| -------------------------- | --------------------------------------------- | ----------------------------------------- |
| The message is             | a key (`$_('greeting')`)                      | a key (`t('greeting')`)                   |
| Ships at runtime           | ICU formatter (`intl-messageformat`) + stores | flattened dictionary + `{{ }}` templating |
| Catalog format             | JSON                                          | JSON or any object                        |
| Plurals                    | ICU, at runtime                               | none built in                             |
| Markup in messages         | strings only                                  | strings only                              |
| Per-locale build           | no                                            | no                                        |
| Locale resolution, routing | none — bring your own                         | none — bring your own                     |
| Per-request SSR binding    | none — one global store                       | none — a signal you provide               |
...
`.output/public/_build`. `/zh/long` is the text-heavy
