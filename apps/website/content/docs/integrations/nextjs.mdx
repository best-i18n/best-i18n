...
Component. A `<Trans>` in a Client Component needs no hook at all: the
compiler gives the message a component of its own to read the locale from.
...
`@best-i18n/next-unprefixed-locale` mirrors `app/[locale]` into a generated
route group with the base locale pinned. That is what this site does — see
[Static export](/docs/integrations/nextjs-static-export).
